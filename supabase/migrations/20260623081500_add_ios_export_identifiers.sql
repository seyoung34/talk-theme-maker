...
add column if not exists theme_identifier text;

do $$
begin
  if not exists (
    select 1 from pg_constraint
    where conname = 'export_jobs_theme_identifier_check'
      and conrelid = 'public.export_jobs'::regclass
  ) then
    alter table public.export_jobs
    add constraint export_jobs_theme_identifier_check
    check (theme_identifier is null or theme_identifier ~ '^com\.kakao\.talk\.theme\.u[0-9a-f]{16}\.i[0-9]{6,}$');
  end if;
end;
$$;

create unique index if not exists export_jobs_theme_identifier_idx
on public.export_jobs (theme_identifier)
where theme_identifier is not null;
