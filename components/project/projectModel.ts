 ...
 * 사용자가 지울 수 있는 후보인가.
 *
 * `upload`은 이번 편집에서 올린 것이다. `template`은 저장된 시스템 템플릿을 열 때 원격 ref에서
 * hydrate되어 돌아온 것으로, 일반 편집에서는 보호하고 관리자 시스템 템플릿 편집에서만 지울 수 있다.
 ...
 * 쓰도록 여기 한 곳에 둔다.
 */
export function isRemovableUploadCandidate(
  candidate: Pick<SlotCandidate, "source" | "inherited">,
  { allowTemplateAssetRemoval = false }: { allowTemplateAssetRemoval?: boolean } = {},
) {
  if (candidate.inherited) return false;
  if (candidate.source === "upload") return true;
  return candidate.source === "template" && allowTemplateAssetRemoval;
}
