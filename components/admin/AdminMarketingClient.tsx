      ...
      <div className="mx-auto w-full max-w-5xl px-5 py-8 md:px-8 md:py-12">
        ...
        <header className="mt-6 flex flex-wrap items-center justify-between gap-3">
          <h1 className="flex items-center gap-1.5 text-[26px] font-semibold tracking-[-0.04em] text-[var(--color-on-surface)]">
            주간 지표
            <InfoTip label="지표 안내">
              분석 쿠키 동의와 무관한 숫자입니다. 클릭은 단축 링크 서버 집계, 나머지는 서비스 운영 기록에서 옵니다.
              클릭과 전환은 서로 연결되지 않습니다 — 누가 어느 캠페인에서 왔는지는 저장하지 않습니다.
            </InfoTip>
          </h1>
        ...
        <section className="mt-6 overflow-hidden rounded-[28px] border border-[#dbe8fb] bg-white/92 shadow-[0_22px_62px_rgba(47,107,191,0.09)]">
              ...
              <table className="w-full min-w-[520px] text-sm">
                <thead>
                  <tr className="border-b border-[#e8eff8] text-[11px] font-black text-[#3d7bd6]">
                    <th className="px-5 py-3 text-left sm:px-7">주 시작</th>
                    <th className="px-3 py-3 text-right">클릭</th>
                    <th className="px-3 py-3 text-right">가입</th>
                    <th className="px-3 py-3 text-right">내보내기</th>
                    <th className="px-5 py-3 text-right sm:px-7">결제</th>
                  </tr>
                </thead>
                <tbody>
                  {(report?.weeks ?? []).map((week, index) => (
                    <tr key={week.weekStart} className={`border-b border-[#f1f5fa] last:border-b-0 ${index === (report?.weeks.length ?? 0) - 1 ? "bg-[#f7fbff] font-bold" : ""}`}>
                      ...
                      <td className="px-3 py-3 text-right tabular-nums">{week.clicks}</td>
          ...
          <p className="border-t border-[#e8eff8] px-5 py-3 text-[11px] font-semibold leading-relaxed text-[#8a99ad] sm:px-7">
            마지막 줄은 이번 주이며 아직 진행 중입니다. 클릭이 늘어난 주에 가입도 늘었는지까지가
            이 표로 말할 수 있는 전부입니다 — 개별 가입이 어느 캠페인에서 왔는지는 알 수 없습니다.
          </p>
        </section>

        <section className="mt-6 overflow-hidden rounded-[28px] border border-[#dbe8fb] bg-white/92 shadow-[0_22px_62px_rgba(47,107,191,0.09)]">
          <h2 className="border-b border-[#e8eff8] px-5 py-4 text-sm font-extrabold text-[var(--color-on-surface)] sm:px-7">캠페인별 클릭</h2>
          {isLoading ? (
            <p className="px-5 py-12 text-center text-sm font-bold text-[#5b6b82]">불러오는 중입니다.</p>
          ) : (report?.campaignClicks.length ?? 0) < 1 ? (
            <p className="px-5 py-12 text-center text-sm font-bold text-[#5b6b82]">아직 클릭이 없습니다. 아래 링크를 뿌리면 집계가 시작됩니다.</p>
          ) : (
            <ul>
              {report?.campaignClicks.map((row) => (
                <li key={row.campaign} className="flex items-center justify-between gap-3 border-b border-[#f1f5fa] px-5 py-3.5 last:border-b-0 sm:px-7">
                  ...
                  <span className="shrink-0 text-sm font-bold tabular-nums">{row.clicks}</span>
                </li>
              ))}
            </ul>
          )}
