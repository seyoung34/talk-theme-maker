...
export default function CreditsClient() {
  ...
  return (
    <main className="min-h-screen bg-[var(--color-background)] text-[var(--color-on-background)]">
      ...
      <div className="w-full max-w-6xl px-5 py-8 mx-auto md:px-8 md:py-11">
        <Link href="/account" className="mb-5 inline-flex items-center gap-1.5 rounded-lg text-sm font-extrabold text-[var(--color-on-surface-variant)] focus-visible:outline-2 focus-visible:outline-offset-2 focus-visible:outline-[var(--color-secondary)]"><ArrowLeft size={17} aria-hidden="true" />마이페이지</Link>
        <header className="mb-7 border-b border-[var(--color-outline-variant)] pb-6">
          <p className="text-xs font-extrabold tracking-[0.14em] text-[var(--color-secondary)]">CREDIT STORE</p>
          <h1 className="mt-2 font-[var(--font-display)] text-[32px] font-semibold tracking-[-0.04em] text-[var(--color-on-surface)]">크레딧 충전</h1>
          <p className="mt-2 text-sm font-medium leading-6 text-[var(--color-on-surface-variant)]">필요한 상품을 선택하고 PayApp에서 결제를 완료하세요. 크레딧은 결제 확인 후 즉시 반영됩니다.</p>
        </header>

        {pageError ? <div className="mb-5 flex items-center justify-between gap-3 rounded-xl border border-[#f1b7b1] bg-[var(--color-error-container)] px-4 py-3 text-sm font-semibold text-[var(--color-on-error-container)]" role="alert"><span className="flex items-center gap-2"><AlertCircle size={17} aria-hidden="true" />{pageError}</span><button type="button" className="underline shrink-0 underline-offset-2" onClick={() => void refreshMe()}>다시 시도</button></div> : null}
        ...
        <div className="grid gap-6 lg:grid-cols-[minmax(0,1fr)_360px] lg:items-start mb-5">
          <section aria-labelledby="product-title">
            <div className="flex items-end justify-between gap-4 mb-4"><div><h2 id="product-title" className="text-lg font-extrabold">충전 상품 선택</h2><p className="mt-1 text-xs font-semibold text-[var(--color-on-surface-variant)]">크레딧에는 유효기간이 없습니다.</p></div><div className="text-right"><span className="text-xs font-bold text-[var(--color-on-surface-variant)]">보유 크레딧</span><strong className="ml-2 text-lg font-extrabold">{isLoading ? "—" : me?.credits ?? 0}</strong></div></div>
            <fieldset className="grid gap-3 sm:grid-cols-3" disabled={chargePhase !== "idle"}>
              ...
              {creditProducts.map((product) => {
                ...
                return (
                  <label key={product.id} className={`relative flex min-h-48 cursor-pointer flex-col rounded-[18px] border bg-white p-5 transition focus-within:outline-2 focus-within:outline-offset-2 focus-within:outline-[var(--color-secondary)] ${selected ? "border-[var(--color-secondary)] shadow-[0_12px_30px_rgba(42,103,103,0.12)]" : "border-[var(--color-outline-variant)] hover:border-[var(--color-outline)]"}`}>
                    ...
                    {"badge" in product ? <span className="mb-6 w-fit rounded-full bg-[var(--color-primary-container)] px-2.5 py-1 text-[10px] font-extrabold text-[var(--color-on-primary-container)]">{product.badge}</span> : <span className="h-5 mb-6" />}
                    <strong className="text-xl font-extrabold">{product.label}</strong>
                    <span className="mt-1 text-sm font-semibold text-[var(--color-on-surface-variant)]">Export {product.credits}회</span>
                    <strong className="pt-5 mt-auto text-xl font-extrabold">{product.amount.toLocaleString("ko-KR")}원</strong>
          ...
          <aside className="rounded-[20px] border border-[var(--color-outline-variant)] bg-white p-5 lg:sticky lg:top-24" aria-labelledby="checkout-title">
            ...
            <dl className="mt-5 grid gap-3 border-y border-[var(--color-outline-variant)] py-4 text-sm">
            ...
            {me?.user ? (
              <>
                ...
                <button type="button" className="mt-4 flex min-h-12 w-full items-center justify-center gap-2 rounded-xl bg-[var(--color-inverse-surface)] px-4 py-3 text-sm font-extrabold text-[var(--color-inverse-on-surface)] transition hover:opacity-90 focus-visible:outline-2 focus-visible:outline-offset-2 focus-visible:outline-[var(--color-secondary)] disabled:cursor-not-allowed disabled:opacity-55" onClick={() => void chargeCredits()} disabled={chargePhase !== "idle" || isLoading}>
              ...
              <div className="mt-5 rounded-xl bg-[var(--color-surface-low)] p-4"><p className="text-sm font-extrabold">결제하려면 로그인이 필요합니다.</p><Link href="/login?returnTo=%2Fcredits&reason=billing" className="mt-3 flex min-h-11 items-center justify-center rounded-xl bg-[var(--color-inverse-surface)] px-4 py-2.5 text-sm font-extrabold text-[var(--color-inverse-on-surface)]">로그인</Link></div>
            )}
        ...
        <section className="mb-6 grid gap-5 rounded-[20px] border border-[var(--color-outline-variant)] bg-white p-5 sm:grid-cols-[minmax(0,1fr)_minmax(280px,0.8fr)] sm:items-center sm:p-6" aria-labelledby="grant-code-title">
          <div className="flex items-start gap-3">
            <span className="grid size-10 shrink-0 place-items-center rounded-xl bg-[var(--color-primary-container)] text-[var(--color-on-primary-container)]"><Gift size={20} aria-hidden="true" /></span>
          ...
          {me?.user ? (
            <form className="grid gap-2" onSubmit={redeemGrantCode} noValidate>
              ...
              <div className="flex gap-2">
                ...
                <button type="submit" className="flex min-h-12 shrink-0 items-center justify-center gap-2 rounded-xl bg-[var(--color-inverse-surface)] px-4 py-3 text-sm font-extrabold text-[var(--color-inverse-on-surface)] focus-visible:outline-2 focus-visible:outline-offset-2 focus-visible:outline-[var(--color-secondary)] disabled:cursor-not-allowed disabled:opacity-55" disabled={isRedeeming || !grantCode.trim()}>{isRedeeming ? <LoaderCircle className="animate-spin" size={17} aria-hidden="true" /> : null}등록</button>
              </div>
          ...
          ) : <Link href="/login?returnTo=%2Fcredits&reason=billing" className="flex min-h-12 items-center justify-center rounded-xl bg-[var(--color-inverse-surface)] px-4 py-3 text-sm font-extrabold text-[var(--color-inverse-on-surface)]">로그인하고 코드 등록</Link>}
        </section>
...
function PaymentNotice({ outcome, onRetry }: { outcome: NonNullable<PaymentOutcome>; onRetry?: () => void }) {
  ...
  return <div className={`mb-6 flex items-start gap-3 rounded-xl border px-4 py-3.5 ${config.className}`} role={outcome.status === "failed" ? "alert" : "status"}><config.Icon className={outcome.status === "checking" ? "mt-0.5 shrink-0 animate-pulse" : "mt-0.5 shrink-0"} size={19} aria-hidden="true" /><div className="flex-1 min-w-0"><p className="text-sm font-extrabold">{config.title}</p><p className="mt-0.5 text-xs font-semibold leading-5">{outcome.message}</p>{outcome.status === "paid" ? <Link href="/account" className="inline-block mt-2 text-xs font-extrabold underline underline-offset-2">마이페이지에서 잔액 확인</Link> : null}</div>{onRetry && (outcome.status === "pending" || outcome.status === "failed") ? <button type="button" className="inline-flex items-center gap-1 text-xs font-extrabold underline shrink-0 underline-offset-2" onClick={onRetry}><RefreshCw size={13} aria-hidden="true" />다시 확인</button> : null}</div>;
}
