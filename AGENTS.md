...
- Anything reachable from `services/android-builder/entrypoint.ts` or
  `services/ios-builder/entrypoint.ts` — which includes `lib/theme/android/buildCore.ts`,
  `lib/theme/export/catalogTransform.ts`, `lib/theme/export/inputArchive.ts`, and
  `services/shared/*`:

  ```powershell
  npx tsc -p services/android-builder/tsconfig.json --noEmit
  npx tsc -p services/ios-builder/tsconfig.json --noEmit
  ```

  Both builders compile with `moduleResolution: NodeNext` and `paths: {}`, so relative imports
  need explicit `.js` extensions and `@/` aliases do not resolve. The root check uses "bundler"
  resolution and catches neither. Keep browser-only code (DOM APIs, `templates.ts`) out of those
  import graphs; shared pure rules belong in modules like `lib/theme/android/rasterPlan.ts`.
  Resolve runtime-only globals such as `FixedLengthStream` through `globalThis` with a local
  structural type instead of a Workers type package, so the same source still compiles here.
