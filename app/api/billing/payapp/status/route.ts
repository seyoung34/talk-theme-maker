...
export async function GET(request: NextRequest) {
  ...
  const { data, error } = await admin.from("payments").select("id,order_id,amount,credits,status,analytics_transaction_id,provider_payment_id,checkout_url,receipt_url,created_at,updated_at").eq("id", paymentId).eq("user_id", user.id).maybeSingle();
