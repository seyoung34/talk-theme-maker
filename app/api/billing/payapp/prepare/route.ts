...
import { creditPack, getCurrentUserOrNull } from "@/lib/billing/credits";
...
export async function POST(request: Request) {
  ...
  if (!body.phone || !isValidKoreanPhone(body.phone)) {
    return NextResponse.json({ error: "휴대폰번호를 정확히 입력해 주세요.", reason: "invalid_phone" }, { status: 400 });
  }

  const admin = createAdminClient();
  const orderId = `credits-${Date.now()}-${crypto.randomUUID()}`;
  ...
  try {
    const checkout = await createPayappCreditCheckout({
    ...
    const { error: updateError } = await admin
      ...
      .update({
        ...
        receipt_url: checkout.receiptUrl,
    ...
    const message = error instanceof Error ? error.message : "페이앱 결제 요청에 실패했습니다.";
    const status = message.includes("PayApp server configuration is missing") ? 503 : 400;
    return NextResponse.json({ error: status === 503 ? "페이앱 환경변수가 설정되지 않았습니다." : message, reason: status === 503 ? "payapp_config_missing" : "payapp_prepare_failed" }, { status });
  }
}
