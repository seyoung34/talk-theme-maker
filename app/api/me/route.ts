...
export async function GET() {
  ...
  if (!user) return NextResponse.json({ user: null, profile: null, credits: 0, exports: [], isAdmin: false });
  ...
  const [profileResult, balanceResult, exportsResult, adminResult] = await Promise.all([
    ...
    admin.from("export_jobs").select("id,platform,export_mode,status,stage,credit_cost,file_name,export_number,application_id,export_name,error,error_code,input_file_count,input_bytes,output_bytes,duration_ms,created_at,completed_at").eq("user_id", user.id).order("created_at", { ascending: false }).limit(10),
  ...
  return NextResponse.json({ user: { id: user.id, email: user.email }, profile: profileResult.data, credits: balanceResult.data?.balance ?? 0, exports: exportsResult.data ?? [], isAdmin: Boolean(adminResult.data) });
}
