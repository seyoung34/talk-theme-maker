...
- [ ] 7. Run full verification and document migration notes
  What to do / Must NOT do: Run all command gates and real-surface QA. Add or keep a reusable QA seed/fixture script only if it is documented and does not expose secrets; otherwise remove temporary seed scripts after evidence is captured. Update `docs/setup/supabase-setup.md` or a short migration note to explain canonical asset management. Do not claim done from green TypeScript alone.
  Parallelization: Wave 3 | Blocked by: 1, 2, 3, 4, 5, 6 | Blocks: final verification
  References: `AGENTS.md:90`; `docs/setup/supabase-setup.md:16`; `docs/architecture/ux-flow.md:29`; `.omo/drafts/admin-assets-canonical-refactor.md`.
