...
- GA 태그는 Consent Mode v2 기본 `denied` 상태로 먼저 로드한다. 동의 전에는 분석 쿠키·페이지뷰·제품 행동 이벤트를 수집하지 않는다.
...
GA4 `관리 → 데이터 표시 → 맞춤 정의`에서 아래 이벤트 범위 측정기준을 등록한다. 이벤트 파라미터명은
코드와 정확히 같게 입력한다. GA4 기본 제공 항목인 거래 ID·통화·금액과 UTM은 중복 등록하지 않는다.

| 표시 이름 | 이벤트 파라미터 |
|---|---|
| 템플릿 키 | `template_key` |
| 템플릿 출처 | `template_source` |
| 대상 플랫폼 | `platform` |
| 내보내기 방식 | `export_mode` |
| 슬롯 역할 | `slot_role` |
| 편집 영역 | `section` |
...
| 최초 랜딩 경로 | `landing_page` |
| 리퍼러 호스트 | `referrer_host` |

`관리 → 데이터 표시 → 이벤트`에서는 `editor_ready`, `export_completed`, `purchase`,
`credit_redeem_completed`를 핵심 이벤트로 지정한다. 사용자·이벤트 데이터 보관 기간은 14개월로 설정한다.
맞춤 측정기준은 등록 뒤 수집되는 데이터부터 보고서에서 사용할 수 있으며 반영까지 24~48시간을 잡는다.

### Looker Studio 일간 페이지

GA4 커넥터로 같은 속성을 연결하고 아래 5개 페이지를 만든다.

1. 개요: 사용자·세션·`editor_ready`·`export_completed`·최근 7일 WEC.
2. 제품 퍼널: `template_viewed → template_started → editor_ready → export_completed` 단계별 사용자와 전환율.
3. 템플릿/플랫폼: `template_key`, `platform`별 시작·완료 이벤트 수와 `완료/시작` 계산 필드.
4. 수익화: 크레딧 차단 → 충전 화면 → 결제 시작 → `purchase`/리딤 → 내보내기 완료.
5. 유입: 기본 세션 소스/매체/캠페인별 에디터 진입·결제·내보내기 완료.

GA4는 동의한 방문만 포함하므로 이 대시보드에서 전체 방문 대비 쿠키 동의율을 계산하지 않는다.

## 구현 및 검증 순서

1. GA4 속성과 웹 데이터 스트림을 만들고 `NEXT_PUBLIC_GA_MEASUREMENT_ID`로 측정 ID를 주입한다.
2. 쿠키 동의 UI와 로컬 동의 상태를 구현한다. 동의 철회 시 이후 분석 이벤트 전송을 중지한다.
3. 외부 태그보다 먼저 기본 `denied`를 큐에 넣고, 동의 후에만 제품 이벤트를 허용하는 공통 래퍼를 만든다.
...
- 동의 전에는 분석 쿠키와 사용자 행동 이벤트가 없고 광고 관련 동의는 항상 `denied`다.
