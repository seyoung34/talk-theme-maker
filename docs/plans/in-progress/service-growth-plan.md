...
> 상태: Phase 1(Consent Mode 기반 GA4·출시 프로모션/결제 퍼널) 구현 진행 중. 배포 환경 수집 검증·GA4 맞춤 측정기준·대시보드 구성이 남아 있다.
...
모두 동의 후에만, 개인 식별 정보·실제 결제 식별자 없이 수집한다. Google 태그는 Consent Mode v2
기본 `denied` 상태로 먼저 로드되며, 동의 전에는 분석 쿠키와 제품 행동 이벤트를 수집하지 않는다.
...
5. **활성화**: 첫 세션 `editor_ready` 도달률(Aha), 유입 채널별 활성화율.
...
- [x] Consent Mode v2 기본 `denied`, 동의 후 페이지뷰·제품 이벤트 전송, 동의 철회 UI를 구현했다.
...
- [x] GA4 속성과 측정 ID `NEXT_PUBLIC_GA_MEASUREMENT_ID`를 설정하고 Measurement Protocol로 스트림 연결을 확인했다.
- [ ] 배포 사이트에서 동의 후 실제 `collect`와 DebugView 핵심 퍼널을 실측한다.
...
1. (검증) 배포 사이트에서 기본 `denied` → 동의 업데이트 → `page_view`/`collect` 순서와 DebugView 반영을 확인한다.
2. (운영) GA4 맞춤 측정기준을 등록하고 Looker Studio에 일간 템플릿·플랫폼·수익화 대시보드를 구성한다.
3. (운영) 인스타그램 링크에는 원문 코드가 아닌 `utm_campaign=instagram_personal_launch`를 사용한다.
4. (보안) 진단용 Measurement Protocol API 비밀번호 `debug-test-secret`을 삭제한다.
