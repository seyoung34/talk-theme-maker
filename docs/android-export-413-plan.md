...
> 상태: Android 옵션 1 구현 완료. 로컬/배포 검증 후 iOS 이식과 원격 에셋 참조화 진행.
...
- **iOS 동일 문제**: `createIosExportFormData`도 같은 구조라 동일 413 위험. Android로 먼저 검증 후 같은
  기법을 iOS에 그대로 이식.
...
1. 로컬/프리뷰에서 Android APK 내보내기 요청 크기와 결과물 동일성 확인.
2. 실기기 확인 통과 후 iOS 내보내기에 동일 기법 이식.
