...
- **GCP 인증** → **v1 SA 키**(Cloudflare 시크릿), 이후 WIF로 전환.
...
- Cloudflare 함수 → GCP: **v1은 SA 키를 Cloudflare 시크릿으로 확정**(셋업 빠름). 앱 로직 변경 없이 뒤에
  **Workload Identity Federation(OIDC)** 으로 전환 예정(장기 키 제거).
...
- **시크릿/환경변수**: GCP(WIF 또는 SA), Supabase 키, 빌드 버킷/Job 설정 → Cloudflare 환경변수·시크릿.
...
- [ ] Cloud Run Job 리소스 + 서비스 계정(GCS 읽기/쓰기, Job 실행 권한) 생성
- [ ] SA 키 발급(v1) → Cloudflare/로컬 시크릿에 보관
...
- [ ] `lib/theme/android/buildJobClient.ts`(엣지 안전): 입력 GCS 업로드 + Job 트리거(SA 키) + `jobId` 반환
...
- [ ] 시크릿/환경변수 이전(SA 키, Supabase, 버킷/Job 설정)
...
### Phase 4 — 후속/정리
- [ ] SA 키 → Workload Identity Federation 전환
